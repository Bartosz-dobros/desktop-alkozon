...
def test_warehouse_item_model():
    item = InventoryProductRow(
        productId=1, name="Piwo 0.5l", quantity=100, warehouseZone="A1"
    )
...
async def test_get_all_items_async(warehouse_service, mocker):
    mock_response = {
        ...
        "rawMaterials": [
            {"id": 1, "name": "Jeczmien", "unit": "kg", "quantity": 500.0}
        ],
    }
    mocker.patch(
        "desktop_alkozon.features.warehouse.service.api_client.get",
